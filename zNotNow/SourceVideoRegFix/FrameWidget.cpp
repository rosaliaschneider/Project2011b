...
void FrameWidget::setHomographies(vector<RX::mat3> *homographies)
{
	_homographies = homographies;
}

void FrameWidget::setBoards(vector< vector<BBox> > *boards)
{
	_boards = boards;
}

void FrameWidget::resizeGL(int w, int h)
{
	_width = w;
	_height = h;
...
void FrameWidget::apply()
{
	if(_isUpdating)
		return;
}

void FrameWidget::mouseMoveEvent(QMouseEvent *ev)
{
	if(_frame == NULL)
		return;
}
