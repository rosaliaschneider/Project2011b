...
: QGLWidget(QGLFormat(QGL::SampleBuffers), parent), _frame1(NULL), _frame2(NULL), _sifts1(NULL), _sifts2(NULL), _selectedP(-1)
{
...
void GLWidget::paintGL()
{
	...
	if (_frame1) 
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, GL_BGRA, GL_UNSIGNED_BYTE, _frame1->bits());
		...
		if(_sifts1)
		{
			...
			glBegin(GL_POINTS);
			{
				for(int i = 0; i < _sifts1->size(); ++i)
				{
					...
					glVertex3f((*_sifts1)[i].pos.x + left1 + 215, top1 - (*_sifts1)[i].pos.y, 1.0);
				}
			}
			glEnd();
			...
			if(_selectedP != -1) 
			{
				glPointSize(7);
				glBegin(GL_POINTS);
				glVertex3f((*_sifts1)[_selectedP].pos.x + left1 + 215, top1 - (*_sifts1)[_selectedP].pos.y, 1.0);
				glEnd();
			}
		}
	...
	if (_frame1 && _frame2) 
	{
		...
		glBegin(GL_POINTS);
		{
			for(int i = 0; i < _sifts1->size(); ++i)
			{
				...
				glVertex3f((*_sifts1)[i].pos.x + left1, top1 - (*_sifts1)[i].pos.y, 1.0);
				glVertex3f((*_sifts2)[i].pos.x + left2, top2 - (*_sifts2)[i].pos.y, 1.0);
			}
		}
		glEnd();
		...
		if(_selectedP != -1) {
			...
			glVertex3f((*_sifts1)[_selectedP].pos.x + left1, top1 - (*_sifts1)[_selectedP].pos.y, 1.0);
			glVertex3f((*_sifts2)[_selectedP].pos.x + left2, top2 - (*_sifts2)[_selectedP].pos.y, 1.0);
...
void GLWidget::mouseMoveEvent(QMouseEvent *ev)
{
	...
	for(int i = 0; i < _sifts1->size(); ++i)
	{
		double pX = (*_sifts1)[i].pos.x + left1 + 215;
		double pY = top1 - (*_sifts1)[i].pos.y;
