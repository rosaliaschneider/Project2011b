...
void Homography::loadPointsFirst()
{
	//_n = siftObj->numMatches(file);
	//
	//createMatrix(_x1, 3, _n);
	//
	//for(int i=0; i < _n; ++i) 
	//{
	//	_x1[0][i] = siftObj->siftPoints(file, siftObj->match(file, i).first, 0);
	//	_x1[1][i] = siftObj->siftPoints(file, siftObj->match(file, i).first, 1);
	//
	//	_x1[0][i] -= _w/2;
	//	_x1[1][i] -= _h/2;
	//
...
void Homography::loadPointsSecond()
{
	//_n = siftObj->numMatches(file);
	//
	...
	//for(int i=0; i < _n; ++i) 
	//{
	//	_x2[0][i] = siftObj->siftPoints(file+1, siftObj->match(file, i).second, 0);
	//	_x2[1][i] = siftObj->siftPoints(file+1, siftObj->match(file, i).second, 1);
	//
	//	_x2[0][i] -= _w/2;
	//	_x2[1][i] -= _h/2;
	//
